...
class DataModule(pl.LightningDataModule):
    ...
    def get_path_loader(self, phase):
        phase_cfg = self.cfg.dataset.get(phase)
        batch_size = phase_cfg.batch_size
        ds = PathDataset(phase, self.cfg)
        # ds = FSDataset_add_STFT(phase, self.cfg)
        dl = DataLoader(ds, 
                        batch_size=batch_size,
                        shuffle=phase_cfg.shuffle,
    ...
    def val_dataloader(self):
        return self.get_path_loader('val')
...
class PathDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        wav = librosa.load(wavpath_full, sr=self.sr)[0] 
        wav = torch.from_numpy(wav)
        length = wav.shape[0]
        # if length < self.min_audio_length:
        #     wav = F.pad(wav, (0, self.min_audio_length - length))
        ...
        feat =  self.feature_extractor(F.pad(wav[0,:].cpu(), (160, 160)), sampling_rate=16000, return_tensors="pt") .data['input_features']
        out = {
